...
class Audio(object):
    ...
    def get_device_count(self) -> int:
        return self._audio.get_device_count()

    def get_device_info_by_index(self, device_index):
        if device_index is not None:
            return self._audio.get_device_info_by_index(device_index)
        else:
    ...
    def print_microphones_info(self):
        Audio._print_microphone_info(self._audio.get_default_input_device_info())
        ...
        for i in range(self._audio.get_device_count()):
            ...
            if device_info["maxInputChannels"] != 0:
                print()
                Audio._print_microphone_info(device_info)

    def create_microphone_stream(self, settings: stream_settings.StreamSettings):
        stream = self._audio.open(input_device_index=settings.device_index,
        ...
        mic_stream = microphone_stream.MicrophoneStream(stream, settings)
